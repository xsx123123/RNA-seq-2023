#!/usr/bin/env R
# PROTEIN MASS downsteam analysis scripts
# date:2023.5.22
# author：zhangjian
# version:4.1.1v
# special:mus (mmu)
# this is an PROTEIN MASS downsteam analysis general scripts
# this is kegg、GO、GSEA scripts for some group gene list
##########################################################################
rm(list=ls())
################################# PATH-1 #################################
...
# Function1:define read htseq-count file function
...
################################# PATH-2 #################################
# PATH-2 set run parameter
print_color_note_type2("input setting !!!!")
args = commandArgs(trailingOnly=TRUE)
# set ROOT NAME
root_dir <- args[1]
# set DEG folder name
deg_file <- "8.deg_exon"
# set save enrichment result folder name
save_name <- "9.KEGG_GO_GASE"
# set kegg special name
KEGG_database <- 'mmu'
# gsea analysis dafult:F
GEEA_ANALYSIS <- F
print_color_note_type2("input setting done 🍻🍻🍻🍻🍻!!!!")
################################# PATH-3 #################################
# PATH3 loading package
print_color_note_type2("loading package !!!!")
# loading package
...
print_color_note_type2("loading package done 🍻🍻🍻🍻🍻!!!!")
################################# PATH-4 #################################
# PATH4 loading function
# print run condition
print_color_note_type2("loading function !!!!")
...
id_convert <- function(gene_list){
  ...
  gene.kegg.entrez_id = mapIds(x = org.Mm.eg.db,keys = gene_list,keytype = "ENSEMBL",column = "UNIPROT")
  # remove NA  gene
...
KEGG <- function(gene_list,name){
  # test parameter
  # gene_list <- pvalue_up_gene_list
  # name <- "pvalue_up_kegg"
  # KEGG analysis
  kegg_id <- id_convert(gene_list)
  save_folder <- file.path(root_dir,save_name,name)
  ...
  if(dim(kegg_result)[1]==0){
    ...
    kegg_result_id <- setReadable(kegg_result, OrgDb = org.Mm.eg.db, keyType="ENTREZID")
...
GO_analysis <- function(gene_list,save_name,save_dir_GO){
  ...
  # enrichGO for BP
  gene.go.BP = enrichGO(gene = gene_list,OrgDb = org.Mm.eg.db,keyType = "ENSEMBL",
  ...
  if (dim(gene.go.BP)[1] == 0){
    ...
    cat("\n")
    save_BP_result <- as.data.frame(setReadable(gene.go.BP, OrgDb = org.Mm.eg.db, keyType="ENSEMBL"))
  }else{
    # ID convert for BP
    save_BP_result <- as.data.frame(setReadable(gene.go.BP, OrgDb = org.Mm.eg.db, keyType="ENSEMBL"))
  ...
  # enrichGO for CC
  gene.go.CC = enrichGO(gene = gene_list,OrgDb = org.Mm.eg.db,keyType = "ENSEMBL",
  ...
  if (dim(gene.go.CC)[1] == 0){
    ...
    cat("\n")
    save_CC_result <- as.data.frame(setReadable(gene.go.CC, OrgDb = org.Mm.eg.db, keyType="ENSEMBL"))
  }else{
    # ID convert for CC
    save_CC_result <- as.data.frame(setReadable(gene.go.CC, OrgDb = org.Mm.eg.db, keyType="ENSEMBL"))
  ...
  # enrichGO for MF
  gene.go.MF = enrichGO(gene = gene_list,OrgDb = org.Mm.eg.db,keyType = "ENSEMBL",pAdjustMethod="fdr",
  ...
  if (dim(gene.go.MF)[1] == 0){
    ...
    cat("\n")
    save_MF_result <- as.data.frame(setReadable(gene.go.MF, OrgDb = org.Mm.eg.db, keyType="ENSEMBL"))
  }else{
    # ID convert for MF
    save_MF_result <- as.data.frame(setReadable(gene.go.MF, OrgDb = org.Mm.eg.db, keyType="ENSEMBL"))
...
GSEA_KEGG <- function(all_deg_data){
  # id convert
  convert_list <- all_deg_data$SYMBOL
  gene <- bitr(convert_list,fromType = 'SYMBOL',toType = 'ENTREZID',OrgDb = org.Mm.eg.db)
  # remove dup content
  ...
  # sort data
  GSEA_data <- GSEA_data %>% mutate(rank = rank(log2FoldChange,  ties.method = "random")) %>% arrange(desc(log2FoldChange))
  gene_list <- GSEA_data$log2FoldChange
  ...
  if (dim(GSEA_KEGG)[1]==0){
    ...
    GSEA_KEGG_convert_id <- setReadable(GSEA_KEGG, OrgDb = org.Mm.eg.db, keyType="ENTREZID")
    # create GSEA folder
    save_gsea_folder <- file.path(root_dir,save_name,"gsea_enrichment")
...
# print run condition
print_color_note_type2("loading function done 🍻🍻🍻🍻🍻!!!!")
################################# PATH-5 #################################
# PATH5 set save dir
print_color_note_type2("input gene&deg list !!!!")
# save dir obout neuron
save_dir <- file.path(root_dir,save_name)
create_dir(save_dir) 
# input gene list 
print_color_note_type2("input gene&deg list done 🍻🍻🍻🍻🍻!!!!")
################################# PATH-6 #################################
...
deg_folder_name = file.path(root_dir,deg_file)
deg_dir = file.path(deg_folder_name,"output")
pvalue_up_file_name = list.files(deg_dir,pattern = "^L.*p_value_up_output_result.csv$")
pvalue_up_deg_result <- read.csv(file.path(deg_dir,pvalue_up_file_name))
pvalue_up_gene_list <- pvalue_up_deg_result$Row.names
# GO analysis
save_folder_GO_UP <- file.path(root_dir,save_name,"pvalue_up_GO")
create_dir(save_folder_GO_UP)
GO_analysis(pvalue_up_gene_list,"UP",save_folder_GO_UP)
# KEGG analysis
KEGG(pvalue_up_gene_list,"pvalue_up_kegg")
...
deg_folder_name = file.path(root_dir,deg_file)
deg_dir = file.path(deg_folder_name,"output")
pvalue_down_file_name = list.files(deg_dir,pattern = "^L.*_p_value_down_output_result.csv$")
pvalue_down_deg_result <- read.csv(file.path(deg_dir,pvalue_down_file_name))
pvalue_down_gene_list <- pvalue_down_deg_result$Row.names
# GO analysis
save_folder_GO_DOWN <- file.path(root_dir,save_name,"pvalue_Down_GO")
create_dir(save_folder_GO_DOWN)
GO_analysis(pvalue_down_gene_list,"Down",save_folder_GO_DOWN)
# KEGG analysis
KEGG(pvalue_down_gene_list,"pvalue_down_kegg")
# KEGG & GO analysis finish!!!!!!!
print_color_note_type2("down gene kegg analysis (pvalue < 0.5 & log2foldchange <-1) done !!!!")
################################# PATH-8 #################################
# PATH8 GSEA analysis
# get deg folder dir
all_deg_dir <- file.path(root_dir,deg_file,"output")
# get all deg file
all_deg_file <- list.files(all_deg_dir,pattern = "^L.*_DEG_output_result.csv$")
# input all deg file
all_deg_result <- read.csv(file.path(all_deg_dir,all_deg_file))
# extert name & logFC
all_deg_data <- all_deg_result[,c("SYMBOL","log2FoldChange")]
all_deg_data <- na.omit(all_deg_data)
# sort 
all_deg_data <- all_deg_data[order(-all_deg_data$log2FoldChange),]
colnames(all_deg_data) <- c("SYMBOL","log2FoldChange")
# GSEA
if(isFALSE(GEEA_ANALYSIS)){
  cat("GSEA analysis don't analysis")
}else{
  GSEA_KEGG(all_deg_data)
}
################################# finish #################################
print_color_note_type2("all analysis done !!!!")
################################   END   #################################